    ...
    const handleLogin = () => {
      loginFormRef.value.validate(async (valid) => {
        ...
        try {
          ...
          if ([1, 2, 3, 7, 8].includes(roleId.value)) {
...
.login-card {
  width: 400px;
  padding: 30px;
  background-color: rgba(230,226,217,0.3);
...
.login-title {
  ...
  margin-bottom: 50px;
  font-size: 36px;
  color: #D04342; /* 使用 Element Plus 的主色调 */
}
...
.login-button {
  width: 100%;
  background-color: rgba(174,126,140,1); /* Bright green for the login button to stand out */
  color: #FFFFFF;
  border: none;
  font-weight: bold;
}
...
.login-button:hover {
  background-color: #33cc00; /* Darker green on hover */
}
