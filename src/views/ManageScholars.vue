      ...
      >
        ...
        <el-table-column prop="inviterFullname" :label="$t('ViewScholars.table.inviterFullname')">
          <template #default="scope">
            ...
            >
              <span v-html="([4, 5, 6].includes(scope.row.roleId) && !scope.row.inviterFullname) ? `${scope.row.inviterName}<br>未录入` : scope.row.inviterFullname || ''"></span>
            </span>
          </template>
        </el-table-column>
        <el-table-column prop="totalLearningCost" :label="$t('ViewScholars.form.totalLearningCost')">
          <template #default="scope">
            {{ formatCurrency(scope.row.totalLearningCost, scope.row.currency) }}
          </template>
        </el-table-column>
        <el-table-column prop="unPaidMoney" :label="$t('ViewScholars.form.unPaidMoney')">
          <template #default="scope">
            {{ formatCurrency(scope.row.unPaidMoney, scope.row.currency) }}
          </template>
        </el-table-column>
        <el-table-column :label="$t('ViewScholars.form.totalRevenue')">
          <template #default="scope">
...
import { v4 as uuidv4 } from 'uuid'; // 引入 uuid 生成函数
import { ArrowRightBold, ArrowLeftBold } from '@element-plus/icons-vue'; // 引入 expand/collapse icons
...
export default {
  ...
  setup() {
    ...
    const viewScholar = (userId) => {
      console.log('Viewing scholar with ID:', userId);
